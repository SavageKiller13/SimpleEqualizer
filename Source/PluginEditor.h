...
struct FFTDataGenerator {
    ...
    */
    ...
    void changeOrder(FFTOrder newOrder) {
        ...
        // things that need recreating should be created on the help via std::make_unique<>
...
struct AnalyzerPathGenerator {
    /**
    converts 'renderData[]' into juce::Path
    */
    void generatePath(const std::vector<float>& renderData, juce::Rectangle<float> fftBounds, int fftSize, float binWidth, float negativeInfinity) {
        ...
        auto map = [bottom, top, negativeInfinity](float v) {
            return juce::jmap(v, negativeInfinity, 0.0f, float(bottom + 10), top);
        };
...
struct ResponseCurveComponent : juce::Component, juce::AudioProcessorParameter::Listener, juce::Timer {
    ...
    void toggleAnalyzerView(bool enabled) {
...
private:
    ...
    ButtonAttachment lowCutBypassButtonAttachment, highCutBypassButtonAttachment, peakBypassButtonAttachement, analyzerEnabledButtonAttachment;
